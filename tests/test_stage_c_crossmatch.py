...
def test_run_stage_c_empty_catalog(tmp_path):
...
def test_run_stage_c_all_matched(tmp_path):
    """5 sources, master returns 5 nearby matches → all master_matched=True."""
    cat_path = tmp_path / "catalog.fits"
    out_dir = tmp_path / "out"
    out_dir.mkdir()
    ...
    with patch("dsa110_continuum.catalog.stage_c._cone_search", return_value=master_df):
        ...
        out_path = run_stage_c(
            cat_path, out_dir, ra_center=344.1, dec_center=16.15, radius_deg=1.0
        )
...
def test_run_stage_c_new_source_candidate(tmp_path):
...
def test_run_stage_c_low_snr_not_candidate(tmp_path):
...
def test_output_fits_columns(tmp_path):
...
def test_run_stage_c_no_master_fallback_nvss(tmp_path):
...
def test_output_path_default_stem(tmp_path):
    ...
    expected_name = cat_path.stem + "_crossmatched.fits"
    assert Path(str(out_path)).name == expected_name
    assert Path(str(out_path)).exists()


def test_flux_ratio_computed(tmp_path):
...
def test_astrometry_qa_logged(tmp_path, caplog):
