...
def test_derive_promotion_class_no_anchor_returns_pending_review():
    ...
    assert (
        derive_promotion_class("A", "solved", anchor={
            "primary_model": None, "catalog_xmatch": None, "tile_self_consistency": None,
        })
...
class _FakeManifest:
    ...
    command_line: list[str] = field(default_factory=lambda: [
        "/opt/miniforge/envs/casa6/bin/python",
        "scripts/batch_pipeline.py",
        "--date", "2026-01-25", "--start-hour", "2", "--end-hour", "3",
    ])
    run_log: str = "/data/dsa110-proc/products/mosaics/2026-01-25/run_2026-04-30T00.log"
    epochs: list[dict[str, Any]] = field(default_factory=lambda: [
        {"hour": 2, "n_tiles": 11, "status": "ok", "qa_result": "PASS",
...
def test_emit_for_run_is_idempotent_for_same_sha(tmp_path):
    ...
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    emit_for_run(m, str(products_dir), str(repo_root),
                 cli_invocation=m.command_line, products_root=str(products_root))
    emit_for_run(m, str(products_dir), str(repo_root),
                 cli_invocation=m.command_line, products_root=str(products_root))
    ledger_path = repo_root / "docs" / "validation" / "promotion-log.md"
